...
namespace EventSourcingPoc.Logistics.ConsoleUI
{
    class Program
    {
        ...
        {
            if (!await _app.LogisticsCartReadModelRepository.HasCartAsync(_customerId))
            {
        ...
        {
            var model = await _app.LogisticsCartReadModelRepository.GetAsync(cartId);
        ...
        {
            var hasCartBeenRemovedAfterCheckout = await _app.LogisticsCartReadModelRepository.HasCartAsync(customerId);
